...
def quantize_to_fp8_e5m2(
    ...
    scale = torch.clamp(scale, min=-14, max=15)
    output = (raw_bits & 0x80000000) + ((scale + 127) << 23) + fraction
    output = output.view(torch.float32)

    output = torch.clamp(output, min=-61440, max=61440)
